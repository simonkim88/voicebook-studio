...
class TTSWorker(QThread):
    ...
    def __init__(self, text, output_path, voice="Sohee", instruct_text="", device="cpu",
        ...
        # 있어서 실제 점유를 0.5G 남짓 과소평가한다. 즉 2500 은 여유가 있는 게
        # 아니라 이미 물리 한도에 걸쳐 있었다.
        #
        # 3000 이 2500 과 실효 배치가 같은데도 더 느린 것이 그 증거다 — 계산량이
        # 같은데 느려지는 이유는 메모리 경로밖에 없다. 이득 1.29배는 측정 노이즈
        # 12.5% 대비 여유가 크지 않아서, 브라우저 하나만 띄워도 뒤집힌다.
        # 그래서 확실하게 들어가는 2000 으로 되돌린다.
        #
