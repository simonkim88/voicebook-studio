...
class TTSWorker(QThread):
    ...
    def __init__(self, text, output_path, voice="Sohee", instruct_text="", device="cpu",
        ...
        #
        # 실제 청크는 _chunk_text 가 500자까지 채워 넣어서 평균 390~430자다.
        # 따라서 배치 크기를 실제로 정하는 것은 위의 개수 상한이 아니라 이 예산이다
        # (2000 이면 4~5개에서 잘려 상한 8 에 닿지도 못했다). RTX 4060 Ti 8GB 실측:
        #
        #     예산   실효 배치   RTF          VRAM 피크
        #     2000   4.0         1.76, 1.84   6.51G
        #     2500   6.0         2.40, 2.23   7.26G   (1.29배지만 아래 참고)
        #     3000   6.0         2.26, 2.00   7.54G   (실효가 같은데 오히려 느림)
        #
        # 이 카드의 실사용 예산은 8.0G 가 아니라 약 7.0G 다 — total 7.99G 중
        # 0.78G 를 데스크톱/디스플레이가 상시 점유한다. 게다가 위의 "VRAM 피크"
        # 는 max_memory_allocated() 라 CUDA context 와 allocator 단편화가 빠져
        # 있어서 실제 점유를 크게 과소평가한다 (실측: 할당 5.2G / 예약 10.6G /
        # 장치 7.7G). 즉 2500 은 여유가 있는 게 아니라 이미 한도에 걸쳐 있었다.
        #
        # 아래로도 내려 봤지만 이득이 없었다. 조합을 라운드로빈으로 섞어 새
        # 프로세스 5회씩, 총 15회 측정한 결과 (전 회차 동일 조건):
        #
        #     예산   실효   평균 RTF   2000 대비
        #     1200   2.4    0.54       0.62배
        #     1600   3.4    0.73       0.84배
        #     2000   4.0    0.87       1.00배   ← 다섯 라운드 모두 1위
        #
        # 절대값이 낮은 것은 그때 GPU 가 저클럭(700MHz/최대 3105MHz)이었기
        # 때문이고, 비율은 유효하다. 정상 클럭 실행의 2000(RTF 1.78)으로
        # 환산하면 1600 ≈ 1.50 / 1200 ≈ 1.10 인데, 이는 다른 날 독립적으로
        # 잰 1.51 / 1.09 와 1% 안에서 일치한다.
        #
        # 예산을 낮추면 VRAM 유출 위험이 줄 것으로 기대했으나 그렇지 않았다.
        # 15회 전부 예약이 6.4~11.0G 로 정상 범위였다 (붕괴 기준 14G).
        #
        # 다만 별도 5회 측정에서 1회는 예약이 17.5G 까지 부풀며 RTF 가 1.01 로
        # 무너졌다(나머지 1.72~1.82). 그때만 GPU 사용률·전력·클럭이 함께 올라
        # 갔다 — 계산이 아니라 PCIe 전송에 시간을 쓴 모습이다. 생성 오디오도
        # 그 회차가 가장 길었으니, 확률적 샘플링이 긴 시퀀스를 뽑으면 KV 캐시가
        # 커져 작업 집합이 카드 밖으로 밀려나는 경로로 보인다. 예산으로는 못
        # 막았고, 진짜 손잡이는 아래 '주의' 의 폴백을 되살리는 쪽이다.
        #
        # 주의: Windows NVIDIA 드라이버는 VRAM 이 모자라면 OutOfMemoryError 를
        # 내지 않고 호스트 RAM 으로 흘려보낸다. 그래서 _generate_batch 의 재귀
        # 이등분이 돌지 않고 조용히 느려지기만 한다. 예산은 넉넉히 잡지 말 것.
        # VRAM 이 더 큰 GPU 라면 올릴 값어치는 있다 — 예산 4000(실효 8.0)에서
        # RTF 2.28 까지 올라갔지만 10.25G 를 요구해 이 카드에서는 못 쓴다.
    ...
    def _effective_batch_size(self):
        ...
        여러 개를 만들 수 있다. RTX 4060 Ti 8GB / 0.6B 에서 실제 책의 청크로
        재면 실효 1.0 → 4.0 에서 RTF 0.61 → 1.80 (2.9배)다.

        여기서 돌려주는 값은 어디까지나 **개수 상한**이고, 실제로 배치 크기를
        정하는 것은 대개 batch_chars 예산 쪽이다 — 실제 청크가 평균 400자라
        예산 2000 이면 4~5개에서 잘려 이 8 에 닿지 않는다. 짧은 청크로 이뤄진
        문서에서만 이 상한이 의미를 갖는다.

        (이전 주석에 있던 B=1 0.281 → B=8 0.897 표는 벤치마크가 64자짜리
        문장을 쓰던 시절의 값이다. 실제 청크의 6분의 1 길이라 예산에 걸리지
        않았고, 길이 편차도 없어 배치 이득이 과대평가돼 있었다. 지금
        bench_gpu.py 는 실제 문서를 _chunk_text 에 태워 쓴다.)
